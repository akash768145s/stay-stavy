...
import { Car, Clapperboard, Dumbbell, MapPin, ShoppingBasket, Utensils, Waves, Wine } from "lucide-react";
...
const HotelCard = ({ hotel }: { hotel: HotelWithRooms }) => {
    ...
    return (
        <div onClick={() => !isMyHotels && router.push(`/hotel-details/${hotel.id}`)} className={cn(
            ...
            <div className="flex gap-2 bg-background/50 border border-primary/10 rounded-lg">
                ...
                <div className="flex-1 flex flex-col justify-between h-[210px] gap-1 p-1 py-2 text-sm">
                    ...
                    <div className="text-primary/90">
                        <AmenityItem><MapPin className="w-4 h-4" />{country?.name},{hotel.city}</AmenityItem>
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-1">
                            {hotel?.rooms[0]?.zoomPrice && <>
                                <div className="font-semibold text-base ">₹{hotel?.rooms[0].zoomPrice}</div>
